...
setwd("F:/Rwork/02_OutAna/kimgoeun_seoul_phD2024/kge_2nd_analysis")
...
setwd("F:/Rwork/02_OutAna/kimgoeun_seoul_phD2024/kge_2nd_analysis")
source("F:/Rwork/02_OutAna/kimgoeun_seoul_phD2024/synllabic_function.R")
load(file ="kge_bind0.RData")
kge_bind0
load(file ="kge_bind10.RData")
...
load(file = "accent_pattern.RData")
accent_pattern
...
save(kge_bind0, file ="kge_bind0.RData")
load(file ="kge_bind0.RData")
...
save(kge_bind10, file ="kge_bind10.RData")
write_excel_csv(kge_bind10, file ="kge_bind10.csv")
load(file ="kge_bind10.RData")
...
#보기
accent_pattern
save(accent_pattern, file = "accent_pattern.RData")
#기본사용
...
kge_bind2 %>% filter(N==1) %>%
  ...
  select(성조, speaker) %>% table() %>%
  # accent_table(cols="speaker", rows = "성조");monosyllabic_word
  accent_table("speaker", "성조")%>% patternGraph(yadd=70)
...
monosyllabic_word
monosyllabic_word%>% data.frame %>% rownames_to_column("accent")
...
kge_bind2 %>% filter(지역 =="부산" & N ==1) %>%
  select(a1, 성조) %>%kge_chisq_table(v1="a1", v2="성조")
...
#저장
aaa = kge_bind2 %>% filter(지역 =="부산" & type=="고유어3") %>%
  auto_pattern("고유어3") %>%
...
data_long <- aaa$chi_table %>%
  ...
  pivot_longer(names_to = "accent", values_to = 'ratio', cols=2:5)

data_long
data_long$accent %>% unique()
...
data_long%>% ggplot(aes(x = accent, y = ratio))+
  ...
  ylim(0,max(data_long$ratio)+0.07)+
  coord_flip()+
  theme_bw()+
...
#
aaa%>% patternGraph1()
aaa$chi_table%>% patternGraph1(raw=F)

#OK
aaa$chi_table %>%
  rownames_to_column("syllabic") %>%
  pivot_longer(names_to = "accent", values_to = 'ratio',
               cols=2:(ncol(aaa$chi_table)+1) )
               # cols=2:5 )


aaa$chi_table %>%   patternGraph1(type=FALSE)
aaa$chi_table%>% patternGraph1(raw=F)
...
kge_bind2 %>% filter(지역 =="부산" & type == "고유어2") %>%
  auto_pattern("고유어2") %>%
  kge_chisq_table("a1","성조", "고유어2 부산 종성")
...
#빈도표 보기 모자이크 플롯 ----
kge_bind2 %>% filter(지역 =="부산" & type == "고유어2") %>%
  auto_pattern("고유어2") %>%  select(성조, a1) %>% table()%>%
  accent_table("성조", "a1", type="all")

kge_bind2 %>% filter(지역 =="부산" & type == "고유어3") %>%
  auto_pattern("고유어3") %>%  select(성조, a1) %>% table()%>%
  accent_table("성조", "a1", type="g")


kge_bind2 %>% filter(지역 =="부산" & type == "고유어2") %>%
  auto_pattern("고유어2") %>%  select(성조, a1) %>% table()%>%
  accent_table("성조", "a1") %>%
  mosaicplot(color = T, ylab = "onset", xlab="accent")
...
# 마찬가지로 초성, 종성을 합쳐서 살펴서 보았을 때 초성, 종성 중 어떤 것이 성조형과 더 밀접하게 관련이 있는지
kge_bind2 %>% filter(지역 =="부산" & type == "고유어2") %>%
  select(성조,a1, a3) %>% unite(onse, a1:a3) %>%
  table() %>% t()
...
kge_bind2 %>% filter(지역 =="부산" & type == "고유어2") %>%
  select(성조,a1, a3) %>% unite(onse, a1:a3) %>%
  table() %>% t() %>% ca::ca() %>% plot( arrows=c(FALSE, T))
  ?ca::ca
  #FactoMineR::CA()
...
## 데이터의 결합 없이 설정
kge_bind2 %>% filter(지역 =="부산" & type == "고유어2") %>%
  select(성조,a1, a2) %>% unite(onse, a1:a2) %>%
  table() %>% t() %>% ca::mjca()
...
##e
kge_bind2 %>% filter(지역 =="부산" & type == "고유어2") %>% data.frame()

#데이터 정리
kge_bind2 %>% filter(지역 =="부산" & type == "고유어2") %>%
  select(성조,a1, a2) %>% unite(onse, a1:a2) %>%
  table() %>% data.frame() %>% separate(onse, c("onset", "coda"),sep="_")

##CA----------
kge_bind2 %>% filter(지역 =="부산" & type == "고유어2") %>%
  select(성조,a1, a2) %>%
  xtabs(formula = ~ 성조+a1+a2, data=.)
...
kge_bind2 %>% filter(지역 =="부산" & type == "고유어2") %>%
  select(type, 성조, 지역, age, A1, a1, A2,a2)

kge_bind2 %>% filter(지역 =="부산" & type == "고유어2") %>%
  select(speaker, word, type, 성조, 지역, age, A1, a1, A2,a2)
...
# kge_bind2 %>% filter(지역 =="부산" & type == "고유어3") %>%
#   auto_pattern("고유어3") %>%
#   # kge_chisq_table("a1","성조", "고유어3 부산 초성", type="res1") %>%
#   kge_accent_table()
#   long_df()
#   patternGraph1()
...
fb3 <-kge_bind2 %>% filter(지역 =="부산" & type == "외래어3") %>%
  auto_pattern("외래어3") %>%
  kge_chisq_table("a1","성조", "외래어3 부산 초성")


fb3$chi_table %>% long_df()
aov(freq ~ accent, fb3$chi_table %>% long_df()) %>% summary()
aov(freq ~ speaker, fb3$chi_table %>% long_df()) %>% summary()
aov(freq ~ accent+speaker, fb3$chi_table %>% long_df()) %>% summary()
...
kge_bind2 %>% filter(지역 =="부산" & type == "외래어3") %>%
  auto_pattern("외래어3") %>%
  kge_chisq_table("w1f","성조", "외래어3 부산 종성 light/heavy", yadd = 0.2)
...
kge_bind2 %>% filter(지역 =="부산" & type == "외래어4") %>%
  auto_pattern("외래어4") %>%
  kge_chisq_table("a3","성조", "외래어4 부산 첫음절 종성 3(light,obs,son",
                  yadd =0.3)
...
kge_bind2 %>% filter(지역 =="부산" & type == "가상어3") %>%
  auto_pattern("가상어3") %>%
  kge_chisq_table("a3","성조", "가상어3 부산 첫음절 종성 3(light,obs,son",
                  yadd =0.18)
